...
def test_get_module_list_success(client, session, create_dummy_modules, master_token):
    """ 모듈 목록 조회 성공 테스트"""
    modules = create_dummy_modules(3)
    
    # WHEN: /admin/modules 엔드포인트를 GET 요청
    response = client.get(
        "/api/admin/modules?page=1&pageSize=10",
...
def test_get_module_list_unauthorized(client):
    ...
    response = client.get("/api/admin/modules?page=1&pageSize=10")
...
def test_get_module_list_pagination(client, session, create_dummy_modules, master_token):
    """ 페이지네이션 테스트"""
    # 기존 모듈 데이터 삭제
    ...
    response1 = client.get(
        "/api/admin/modules?page=1&pageSize=3",
        headers={"Authorization": f"Bearer {master_token}"}
    )
    response2 = client.get(
        "/api/admin/modules?page=2&pageSize=3",
        headers={"Authorization": f"Bearer {master_token}"}
    )
    response3 = client.get(
        "/api/admin/modules?page=3&pageSize=3",
        headers={"Authorization": f"Bearer {master_token}"}
    )
...
def test_get_module_list_invalid_page(client, master_token):
    ...
    response = client.get(
        "/api/admin/modules?page=0&pageSize=10",
        headers={"Authorization": f"Bearer {master_token}"}
    )
...
def test_get_module_list_invalid_page_size(client, master_token):
    ...
    response = client.get(
        "/api/admin/modules?page=1&pageSize=0",
        headers={"Authorization": f"Bearer {master_token}"}
    )
    # THEN: 유효성 검사 실패로 422 에러가 발생함
    assert response.status_code == 422
