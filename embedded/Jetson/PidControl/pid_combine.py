...
def reverse_until_nfc(module_name="module"):
    ...
    original_filename = f"{module_name}_combine_{timestamp}.mp4"  # 원본 녹화 파일
    fixed_filename = f"{module_name}_combine_{timestamp}_fixed.mp4"  # 변환된 파일
    ...
    fps = 20.0  # fps 설정
    out = cv2.VideoWriter(original_filename, fourcc, fps, (frame_width, frame_height))
    ...
    while True:
        ...
        if key == ord('n'):
            ...
            time.sleep(2)
            out.release()
            print(f"✅ 녹화 저장 완료: {original_filename}")

            # ffmpeg를 사용하여 moov atom을 파일 앞쪽으로 이동
            ffmpeg_command = [
                "ffmpeg", "-i", original_filename, "-c", "copy", "-movflags", "+faststart", fixed_filename
            ]
            subprocess.run(ffmpeg_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            
            print(f"✅ 변환 완료 (moov atom 이동): {fixed_filename}")
            break
...
# =============================================================================
# [사용자 입력: 목표 마커 크기, 속도, 루프 주기, 그리고 장착할 마커 ID]
# =============================================================================
# -------------------------------
...
try:
    while True:
        ...
        if ids is not None:
            ...
            elif time.time() - lost_target_time > 2.0:
