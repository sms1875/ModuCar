...
# -----------------------------------------------------------------------------
# [NFC 이벤트] : 'n' 키 대신 소켓을 통해 NFC 신호를 받기 위한 Event
# -----------------------------------------------------------------------------
nfc_detected_event = threading.Event()  # NFC 감지 시 set()

# --- (새로 추가된 부분) 초음파 센서값 등 최신 센서 데이터를 저장할 전역 변수들
latest_nfc_uid = None
...
# =============================================================================
# 4) 카메라 설정 (ArUco + PID 제어) - "정렬 로직"은 기존대로
# =============================================================================
...
# =============================================================================
# [NFC 서버 스레드] : 라즈베리 파이에서 센서 데이터를 받는 예시
...
def nfc_server_thread(host="0.0.0.0", port=5000):
    ...
    예: {"NFC_UID": "04 A1 BC ..", "ULTRASONIC_FRONT": 12.3, "ULTRASONIC_BACK": 15.4}
    NFC_UID가 들어오면 nfc_detected_event.set() 호출.
    """
    print(f"[NFC 서버] {host}:{port}에서 대기 중...")
    server_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server_sock.bind((host, port))
    server_sock.listen(1)
    print("[NFC 서버] 클라이언트 연결 대기...")

    client_sock, addr = server_sock.accept()
    print(f"[NFC 서버] 연결됨: {addr}")

    # 전역 변수에 접근하기 위해 global 지정
    ...
    while True:
        try:
            ...
            # 여러 줄이 한꺼번에 들어올 수 있으므로 줄 단위로 split
            for line in data.splitlines():
                try:
                    ...
                    # (디버그용) 출력
                    # print(f"[NFC 서버] 수신: NFC_UID={nfc_uid}, ULTRASONIC_FRONT={front_distance}, ULTRASONIC_BACK={back_distance}")

                    # NFC_UID가 있으면 이벤트 발생
                    if nfc_uid:
                        print(f"[NFC 서버] NFC 감지됨: {nfc_uid} → nfc_detected_event.set()")
...
def rent_until_nfc(rentId, module_name, ws_send):
    ...
    electromagnet_on()
    motor_hat.set_throttle(-0.4)  # 천천히 후진
    ...
    print("녹화 시작 중... (NFC 감지 신호가 오면 즉시 정지)")
    ...
    while True:
        ...
        # ** 키보드 'n' 대신, 소켓 스레드에서 nfc_detected_event 설정 여부만 확인 **
        ...
        # (필요 시) 키보드 q 입력으로 강제 종료 가능
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            print("[rent_until_nfc] 사용자가 q를 눌러 중단")
            motor_hat.set_throttle(0)
            out.release()
            return
...
def return_until_nfc(rentId, module_name, ws_send):
    ...
    while True:
        ...
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            print("[return_until_nfc] 사용자가 q를 눌러 중단")
            motor_hat.set_throttle(0)
            out.release()
            return
...
# =============================================================================
# 6) "정렬 로직 + 모듈 장착/반납" 메인 함수(스레드용)
# =============================================================================
def rent_process(rent_id, ws_send):
    """
    - 목표 마커 ID(예: 1 또는 2)에 대해 ArUco/PID 정렬
...
def return_process(rent_id, ws_send):
    ...
    while True:
        if stop_event.is_set():
            print("[return_process] 중단 요청 받음 → 종료")
            return
...
def main_loop():
    ...
    while True:
        ...
        if current_thread is None or not current_thread.is_alive():
            ...
            # 현재 무언가 동작 중에 새 명령이 오면 → 중단 후 재시작
...
if __name__ == "__main__":
    try:
        # (1) NFC 서버 스레드 시작 (초음파 센서값도 함께 수신)
